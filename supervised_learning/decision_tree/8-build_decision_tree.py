...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """Get all possible split thresholds for a given feature.
        ...
        Returns:
            numpy.ndarray: Mid-points between unique feature values.
        """
        values = np.unique((self.explanatory[:, feature])[node.sub_population])
        return (values[1:] + values[:-1]) / 2

    def Gini_split_criterion_one_feature(self, node, feature):
        """Evaluate Gini impurity for all thresholds of a single feature.
        ...
        Returns:
            tuple: (best_threshold, minimum_gini_index)
        """
        values = self.explanatory[node.sub_population, feature]
        thresholds = self.possible_thresholds(node, feature)
        labels = self.target[node.sub_population]
        all_labels = np.unique(self.target)
        Left_F = ((values.reshape(-1, 1, 1) > thresholds.reshape(1, -1, 1)) &
                  (labels.reshape(-1, 1, 1) == all_labels.reshape(1, 1, -1)))
        counts_total_nodes = (labels.reshape(-1, 1) ==
                              all_labels.reshape(1, -1)).sum(axis=0)
        counts_left = Left_F.sum(axis=0)
        counts_right = counts_total_nodes - counts_left
        n_left = counts_left.sum(axis=1)
        n_right = counts_right.sum(axis=1)
        probas_left = counts_left / n_left.reshape(-1, 1)
        probas_right = counts_right / n_right.reshape(-1, 1)
        gini_left = np.nan_to_num(1 - (probas_left**2).sum(axis=1))
        gini_right = np.nan_to_num(1 - (probas_right**2).sum(axis=1))
        gini_average = (((n_left / len(values)) * gini_left) +
                        ((n_right / len(values)) * gini_right))
        return (thresholds[gini_average.argmin()],
                gini_average[gini_average.argmin()])

    def Gini_split_criterion(self, node):
        """Determine the best split among all features using Gini impurity.
